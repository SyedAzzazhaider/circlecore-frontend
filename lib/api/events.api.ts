...
export var eventsApi = {
  ...
  rsvp: function(eventId: string) {
    return api.post<ApiResponse<{ rsvpCount: number; hasRsvped: boolean }>>(
      "/events/" + eventId + "/rsvp", {}
    );
  },

  cancelRsvp: function(eventId: string) {
    return api.delete<ApiResponse<{ rsvpCount: number; hasRsvped: boolean }>>(
      "/events/" + eventId + "/rsvp"
    );
  },

  setReminder: function(eventId: string) {
    return api.post<ApiResponse<{ reminderSet: boolean }>>(
      "/events/" + eventId + "/reminder", {}
    );
  },

  removeReminder: function(eventId: string) {
    return api.delete<ApiResponse<{ reminderSet: boolean }>>(
      "/events/" + eventId + "/reminder"
    );
  }
};
