...
var OAUTH_PROVIDERS = [
  ...
  { id: "linkedin", label: "Continue with LinkedIn",  icon: LINKEDIN_ICON },
...
export default function LoginPage() {
  ...
  var isTimeout  = params.get("reason") === "timeout";
  var registered = params.get("registered");

  var onSubmit = async function(data: LoginFormData) {
    try {
      var recaptchaToken = await getToken("login");
      var { data: res } = await authApi.login({
  ...
  return (
    <div className="auth-card">
      ...
      {/* Alert banners */}
      {isTimeout && (
        <div className="mb-6 flex items-start gap-2.5 p-3.5 rounded-xl bg-amber-500/12 border border-amber-500/25">
      ...
      {registered && (
        <div className="mb-6 flex items-start gap-2.5 p-3.5 rounded-xl bg-emerald-500/12 border border-emerald-500/25">
      ...
      <div className="mb-7">
        ...
        <p className="text-[10px] font-bold text-indigo-400 tracking-widest uppercase mb-1.5">Welcome back</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
        ...
        <p className="text-center text-xs text-slate-600 pt-1">
          ...
          <a href="https://policies.google.com/privacy" target="_blank" rel="noopener noreferrer"
            className="text-slate-500 hover:text-slate-400 underline underline-offset-2 transition-colors">Privacy</a>
          &nbsp;&amp;&nbsp;
          <a href="https://policies.google.com/terms" target="_blank" rel="noopener noreferrer"
            className="text-slate-500 hover:text-slate-400 underline underline-offset-2 transition-colors">Terms</a>
        </p>
      </form>
    </div>
  );
