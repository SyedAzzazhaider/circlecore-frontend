...
import { Users, Hash, Lock, Loader2, ArrowLeft, MessageSquare, CheckCircle2, ChevronRight, TrendingUp } from "lucide-react";
import { communityApi, type Community, type Channel } from "@/lib/api/community.api";
import { type Post } from "@/lib/api/feed.api";
...
var CHANNEL_ICONS = { announcement: "📢", resource: "📚", text: "" };
  ...
  if (error || !community) return (
    <div className="max-w-xl mx-auto px-4 py-16 text-center">
      <div className="card p-12">
        ...
        <p className="text-base font-black text-surface-900 mb-1.5">Community not found</p>
        <p className="text-sm text-surface-400 mb-6">{error}</p>
        <button onClick={function () { router.push("/communities"); }}
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold bg-surface-100 hover:bg-surface-200 text-surface-700 border border-surface-200 transition-colors">
    ...
    <div className="flex h-screen overflow-hidden bg-surface-50">
      <aside className="hidden md:flex w-60 shrink-0 bg-white border-r border-surface-200 flex-col">
        <div className="shrink-0">
          ...
          <div className="px-4 -mt-6 pb-4">
            <div className="flex items-end gap-2.5 mb-3.5">
              <div className="w-11 h-11 rounded-2xl flex items-center justify-center text-white text-sm font-black ring-2 ring-white shadow-lg"
              ...
              <div className="min-w-0 flex-1 pb-0.5">
                <h2 className="text-xs font-black text-surface-900 truncate leading-tight">{community.name}</h2>
                <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                  <span className="flex items-center gap-1 text-[10px] text-surface-400 font-semibold">
                    <Users size={9} />{formatCount(community.memberCount)}
                  </span>
                  {community.isPrivate && (
                    <span className="flex items-center gap-0.5 text-[10px] text-surface-400 font-semibold">
                      <Lock size={9} />Private
                    </span>
                  )}
                  {community.isMember && (
            ...
            {community.isMember ? (
              ...
              <button onClick={handleJoin} disabled={joining}
                className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold text-white transition-all hover:shadow-md hover:opacity-90"
          ...
          <div className="mx-4 mb-3 p-3 rounded-xl border border-surface-100 bg-surface-50">
            <div className="grid grid-cols-2 gap-2 text-center">
              <div>
                <p className="text-sm font-black text-surface-900">{formatCount(community.memberCount)}</p>
                <p className="text-[10px] text-surface-400 font-medium">Members</p>
              </div>
              <div>
                <p className="text-sm font-black text-surface-900">{formatCount(community.postCount || 0)}</p>
                <p className="text-[10px] text-surface-400 font-medium">Posts</p>
              </div>
            </div>
          </div>
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-3">
          <p className="text-[9px] font-black text-surface-400 uppercase tracking-widest px-3 mb-2.5">Channels</p>
          {community.channels.length === 0 ? (
            <p className="text-xs text-surface-400 px-3 py-2">No channels yet.</p>
          ) : community.channels.map(function (channel) {
            var isActive = activeChannel && activeChannel._id === channel._id;
            var typeIcon = CHANNEL_ICONS[channel.type] || "";
            return (
              <button key={channel._id}
                onClick={function () { setActiveChannel(channel); }}
                className={["w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-xs transition-all duration-150 text-left mb-0.5",
                  isActive ? "bg-brand-50 text-brand-700 font-bold" : "text-surface-600 hover:bg-surface-100 hover:text-surface-900 font-medium"
                ].join(" ")}>
                {typeIcon
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="shrink-0 bg-white/95 backdrop-blur-md border-b border-surface-200 px-5 py-3 flex items-center justify-between gap-4">
            ...
            {activeChannel ? (
              <>
                <div className="w-8 h-8 rounded-xl bg-brand-50 border border-brand-100 flex items-center justify-center shrink-0">
                  {CHANNEL_ICONS[activeChannel.type]
                    ? <span className="text-sm">{CHANNEL_ICONS[activeChannel.type]}</span>
                ...
                <div className="min-w-0">
                  ...
                  {activeChannel.description && (
                    <p className="text-[11px] text-surface-400 truncate mt-0.5">{activeChannel.description}</p>
                  )}
                </div>
              </>
            ) : (
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-surface-50 border border-surface-200">
              <Users size={11} className="text-surface-400" />
            ...
            {community.isMember ? (
              ...
              <button onClick={handleJoin} disabled={joining}
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold text-white transition-all hover:shadow-md hover:opacity-90"
                style={{ background: "linear-gradient(135deg,#4f46e5,#7c3aed)" }}>
                {joining
                  ? <span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" />
          ...
          <div className="max-w-2xl mx-auto px-4 py-6">
            ...
            {!community.isMember && (
              <div className="mb-5 rounded-2xl overflow-hidden"
                style={{ background: "linear-gradient(135deg,#eef2ff 0%,#f0f9ff 100%)", border: "1px solid #c7d2fe" }}>
                <div className="p-5 flex items-center justify-between gap-4">
                  <div>
                    <p className="text-sm font-black text-surface-900 mb-1">Join to participate</p>
                    <p className="text-xs text-surface-500 leading-relaxed">Become a member to post, comment, and connect.</p>
                  </div>
                  <button onClick={handleJoin} disabled={joining}
                    className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-bold text-white shrink-0 transition-all hover:shadow-md hover:opacity-90"
                    style={{ background: "linear-gradient(135deg,#4f46e5,#7c3aed)" }}>
                    {joining
                      ? <span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" />
